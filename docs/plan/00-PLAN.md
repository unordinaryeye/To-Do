...
> 진행 상태 (2026-09-09): R0 배포 완료 · R1 배포 완료 · R2 구현·리뷰 반영 완료(브랜치 feat/r2, 푸시 대기) · R3 미착수.
> R2에서 계획과 달라진 점: 습관 설정 수정은 "그날부터 이후 정책에도 적용, 상태(쉬는 중/재개 예약)는 유지" 계약으로 확정. 4분면 드래그 이동은 피커 시트로 대체(드래그는 순서변경 페이지에만).
