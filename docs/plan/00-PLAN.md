...
> 진행 상태 (2026-09-09): R0·R1·R2 배포 완료 · R3 구현·리뷰 반영 완료(브랜치 feat/r3, 푸시 대기). R3에서 뺀 것: 미니/플러스/맥스 난이도, 루틴 묶음, 월 보기. 사용자 피드백으로 추가된 것: 탭바 가운데 + 버튼(떠 있는 FAB 제거), 시간순 자동 정렬(설정), 필터 칩 줄의 태그 관리, 내 기록 → 통계 링크. '투두 탭에도 표시' 토글은 사용자 요청으로 제거(루틴은 루틴 탭에만).
